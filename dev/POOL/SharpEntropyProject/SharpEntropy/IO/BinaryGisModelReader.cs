...
namespace SharpEntropy.IO
{
	...
	public class BinaryGisModelReader : GisModelReader
	{
		...
		{
            cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "Starting ReadPredicates", cypher.LogEnum.Debug);
            ...
            cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "outcomePatternCount : " + outcomePatternCount.ToString(), cypher.LogEnum.Debug);
			//read from the model how many predicates there are
            predicates = new Dictionary<string, PatternedPredicate>(ReadInt32());
            cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "Predicates Defined", cypher.LogEnum.Debug);
			...
			{
                cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "currentOutcomePattern : " + currentOutcomePattern.ToString(), cypher.LogEnum.Debug);
                ...
                cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "Loop 1 variables defined", cypher.LogEnum.Debug);
				//read in the outcomes for this pattern
				for (int currentOutcome = 0; currentOutcome <currentOutcomePatternLength; currentOutcome++)
				{
                    cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "In Loop 2", cypher.LogEnum.Debug);
				...
				{
                    cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "Starting loop 3", cypher.LogEnum.Debug);
					...
					{
                        cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "Starting loop 4", cypher.LogEnum.Debug);
                        parameters[currentParameter] = ReadDouble();
					}
                    cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "Adding predicate", cypher.LogEnum.Debug);
					predicates.Add(predicateName, new PatternedPredicate(currentOutcomePattern, parameters));
				}
                cypher.Log.WriteToLog(cypher.info.ProjectInfo.ProjectLogType, "ReadPredicates", "Ending loop 1", cypher.LogEnum.Debug);
			}
		}
	}
}
